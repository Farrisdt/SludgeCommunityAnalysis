---
title: "Sludge Community Analysis"
output: html_document
authors: "Leyla Cufurovic and Ruben Lancaster"
date: "2023-10-23"
editor_options: 
...
| extrafont | fonts for data visualization |
| showtext | fonts for data visualization |
...
To run without having to re-compute objects, run the following: 

```{r}
load("sludge_workspace.RData")
```
...
# load packages for general data processing #
library(devtools)
library(tidyverse)
library(tibble)
library(dplyr)
...
# load packages for general data visualization #
library(ggh4x)
library(viridis)
library(ggplot2)
library(gridExtra)

# load packages for microbiome-specific analyses #
library(dada2)
library(phyloseq)
library(Biostrings)
library(DECIPHER)
...
library(vegan)
library(fantaxtic)
library(magrittr) # fantaxtic depend
library(ggnested) # fantaxtic depend

# override select functions from other packages # 
select = dplyr::select
```

```{r Set up directory}

# Note: Fastq files are too large to host on github. #
...
```{r export asv and taxa}
write_tsv(asv_names, "./files/asv_counts.tsv")
write_tsv(as.data.frame(taxa), "./files/taxa_silva_dada2.tsv")
```
...
#prune to 10,000. See data exploration section. #
...
# make a filtered backup for use later with ANCOMBC# 
filtered_phyobj_all = phyobj_all
```
...
p1
ggsave(filename = "shannon_simpson_mediatype.png", plot = p1, path = "./files/images/alpha_diversity", width = 7, height = 7, units = "in")

p1 = plot_richness(ps,x = "System_Type", measures=c("Shannon", "Simpson"), color="System_Type")
p1
ggsave(filename = "shannon_simpson_systemtype.png", plot = p1, path = "./files/images/alpha_diversity", width = 7, height = 7, units = "in")

p1 = plot_richness(ps,x = "Bin", measures=c("Shannon", "Simpson"), color="Bin")
p1
ggsave(filename = "shannon_simpson_bin.png", plot = p1, path = "./files/images/alpha_diversity", width = 7, height = 7, units = "in")
```
...
p1
#Save plot
ggsave(filename = "shannons_bin_anova.png", plot = p1, path = "./files/images/alpha_diversity", width = 7, height = 7, units = "in")
```
...
ggsave(filename = "euler_media.png", plot = p1, path = "./files/images/beta_diversity", width = 7, height = 4, units = "in")
ggsave(filename = "euler_system.png", plot = p2, path = "./files/images/beta_diversity", width = 7, height = 4, units = "in")
ggsave(filename = "euler_bin.png", plot = p3, path = "./files/images/beta_diversity", width = 7, height = 4, units = "in")
```
...
We can visualize the diversity of each individual sample, each system, and each operation type at the class level. Legends are ommitted here due to the number of taxonomic ranks; the next section will reduce the number of taxa graphed and allows legends.
...
p
ggsave(filename = "top10_order_all.png", plot = p, path = "./files/images/relative_abundance", width = 7, height = 7, units = "in")
...
p
ggsave(filename = "top10_order_bin.png", plot = p, path = "./files/images/relative_abundance", width = 8, height = 7, units = "in")
...
p
ggsave(filename = "top3_class_all.png", plot = p, path = "./files/images/relative_abundance", width = 9, height = 7, units = "in")
...
top_grouped <- top_taxa(phyobj_all,
                        ...
                        n_taxa = 10,
                        grouping = "Bin")
p = top_grouped$top_taxa %>% ggplot(aes(x=Bin, y = abundance*100, fill = Order)) +
  geom_bar(stat = "identity")+
  ylab("Percent abundance")+
  ggtitle("Top 10 orders in each Bin")
p
ggsave(filename = "top10_each_order_bin.png", plot = p, path = "./files/images/relative_abundance", width = 8, height = 7, units = "in")


# Top orders of top 10 classes by Bin
...
p = plot_nested_bar(top_nested$ps_obj,
  ...
  facet_wrap(~Bin,
             scales = "free_x",
             ncol = 6) +
  labs(title = "Relative abundances of the top order in the top 10 classes") +
  theme(plot.title = element_text(hjust = 0.5, 
        ...
        legend.key.size = unit(10, 
                               "points"))
p
ggsave("top_order_top10_classes_bin.png", plot = p, path = "./files/images/relative_abundance", width = 7, height = 3, units = "in")
```
...
library(extrafont)
library(showtext)
font_add(family = "skeena", regular = "\\\\wsl.localhost/Ubuntu/home/ruben/bioinfo/sludge/skeena_d.ttf") # you'll need to download skeena_d.ttf, can't host it on github
showtext_auto() 

#Find top 10 across all samples
norm_ps = merge_samples(phyobj_all, "Bin")
norm_ps@sam_data$Bin = factor(norm_ps@sam_data$Bin, levels = c("N and P removal", "P removal", "PHA bioplastics reactor"))
top_asv <- top_taxa(norm_ps, n_taxa = 10, tax_level = "Order")
...
p3 = plot_nested_bar(ps_obj = top_asv$ps_obj,
                     ...
                     merged_clr = "#919191",
                     sample_order = c("N and P removal", "P removal", "PHA bioplastics reactor"),
...
p3
ggsave("top_10_orders_bin.png", plot = p3, path = "./files/images/poster_figures", width = 8, height = 4.5, units = "in", dpi = 300)
```

ANCOM-BC is an analysis for microbiome differential abundance. Here, we'll identify globally differentially abundant taxa. 
...
filtered_ps = subset_taxa(filtered_ps, Kingdom == "Bacteria")
filtered_ps = subset_taxa(filtered_ps, Family != "Mitochondria")
filtered_ps = subset_taxa(filtered_ps, Order != "Chloroplast")
...
```{r globally differentially abundant taxa}
# grab globally differential taxa from ancombc results object #
...
```{r taxon presence/absence by order}

# helpful biostars post for plotting heatmaps from presence/absence data https://www.biostars.org/p/374427/ #

# grab taxons with structural zeroes# 
tab_zero = ancombc_out$zero_ind %>% column_to_rownames(var = "taxon") %>% as.data.frame()

# this is presence/absence. false for struc. 0 is presence, true for struc. 0 is absence # 
tab_zero[tab_zero == "FALSE"] = "Present"
tab_zero[tab_zero == "TRUE"] = "Absent"

# change colnames to be more readable # 
colnames(tab_zero) = c("PHA bioplastics reactor", "N and P removal", "P removal")

# convert to long format for plotting #
tab_zero = tab_zero %>% as.matrix() # convert to matrix before reshaping to long format
plot = reshape2::melt(tab_zero)
...
ggsave("presence_absence_orders.png", plot = p, path = "./files/images/beta_diversity", width = 7, height = 12, units = "in")
```

```{r taxon presence/absence by class}

# rerun by class # 
ancombc_out_class = ancombc2(filtered_ps,
                             fix_formula = "Bin",
                             tax_level = "Class",
                             group = "Bin",
                             struc_zero = TRUE,
...
# grab taxons with structural zeroes# 
tab_zero = ancombc_out_class$zero_ind %>% column_to_rownames(var = "taxon") %>% as.data.frame()
...
p = plot %>% 
  ...
  ggtitle("Presence Absence by Class") +
  theme_bw()
  
ggsave("presence_absence_class.png", plot = p, path = "./files/images/beta_diversity", width = 7, height = 10, units = "in")
```
...
set.seed(100)
#Bray-Curtis distance matrix calculation by bin Type
...
stress = as.character(round(physeq_ord$stress, digits = 3)) # extract stress measurement from ordination object #
stresslab = paste("Stress =", stress) # make label string #

# plot shepard diagram to visualize goodness of fit# 
png(filename="./files/images/beta_diversity/nmds_shepard_stress_diagram.png") # open png to write to
stressplot(physeq_ord) # plot
dev.off() # close device

# The closer the stress is to 0, the better the fit, with 0 being a perfect representation of higher-dimensional data in lower-dimensional space. Generally, measures below 0.1 are ideal, and measures between 0.2-0.3 are acceptable. Measures higher than 0.3 are considered worse. #
...
p1
ggsave("nmds_Bin.png", plot = p1, path= "./files/images/beta_diversity", width = 8, height = 5, units = "in")
...
# edit metadata to reflect filtered samples #
...
bintype$`Pr(>F)`[1] # extract p-value 

# Testing: are centroids of clusters are significantly different?

# 2. PERMDISP #
bd = betadisper(adonis_dist, submeta$Bin)
anova(bd)

# Testing: are the spread of clusters significantly different? 
...
p1 = plot_ordination(
 ...
 geom_point(aes(color = Bin), alpha = 0.95, size = 5) +
  ...
  annotate("text", x = 1.28, y = -2, label = stresslab, size = 12, family = "skeena")+
  annotate("text", x = 1.8, y = -2.5, label = "Non-metric fit, R² = 0.985", size = 12, family = "skeena")+
  theme(plot.background = element_rect(fill = "#F0EEE4"), legend.background = element_rect(fill = "#F0EEE4"))
  

p1
ggsave("nmds_Bin_poster.png", plot = p1, path= "./files/images/poster_figures", height = 5, width = 8, units = "in", dpi = 300)
```

```{r variance stabilized NMDS: by media type}
set.seed(100)
#Bray-Curtis distance matrix calculation by bin Type
...
p1 = plot_ordination(
 ...
 aes(group = Media_Type_1) +
 geom_point(aes(color = Media_Type_1), alpha = 0.95, size = 6) +
  scale_color_manual(values=c("#F2D000", "#F58609", "#A01892"))+
 stat_ellipse(type = "t", geom = "polygon", position = "identity", alpha = .05, aes(fill = Bin)) + 
 scale_fill_manual(values=c("#F2D000", "#F58609", "#A01892")) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
    panel.background = element_blank(), axis.line = element_line(colour = "black"),strip.text = element_text(colour = 'white'),text = element_text(size=12),legend.text=element_text(size=12),legend.title = element_blank())+
  annotate("text", x = 2, y = -2, label = stresslab) + 
  annotate("text", x = 2, y = -2.5, label = "Non-metric fit, R² = 0.985")
...
p1
ggsave("nmds_Mediatype.png", plot = p1, path= "./files/images/beta_diversity", width = 8, height = 5, units = "in")

```

```{r variance stabilized NMDS: by system scale}
set.seed(100)
#Bray-Curtis distance matrix calculation by bin Type
...
p1 = plot_ordination(
 ...
 aes(group = System_Type) +
 geom_point(aes(color = System_Type), alpha = 0.95, size = 6) +
  scale_color_manual(values=c("#347F92", "#A6CEE3","#94DE11" ))+
 stat_ellipse(type = "t", geom = "polygon", position = "identity", alpha = .05, aes(fill = Bin)) + 
 scale_fill_manual(values=c("#347F92", "#A6CEE3","#94DE11" )) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
    panel.background = element_blank(), axis.line = element_line(colour = "black"),strip.text = element_text(colour = 'white'),text = element_text(size=12),legend.text=element_text(size=12),legend.title = element_blank())+
  annotate("text", x = 2, y = -2, label = stresslab) + 
  annotate("text", x = 2, y = -2.5, label = "Non-metric fit, R² = 0.985")
...
p1
ggsave("nmds_Systemtype.png", plot = p1, path= "./files/images/beta_diversity", width = 8, height = 5, units = "in")

```

```{r variance stabilized NMDS: by year}
set.seed(100)
#Bray-Curtis distance matrix calculation by bin Type

# why is eub12 set to 2024???? something got messed up
...
p1
ggsave("nmds_collectiondate.png", plot = p1, path= "./files/images/beta_diversity", width = 8, height = 5, units = "in")
```
...
# reset phyloseq object # 
phyobj_all = backup_phyobj_all

# normalize # 

# Create a DESeq2 object. Selected experimental variable is Bin (expected function)
diagdds <- phyloseq_to_deseq2(phyobj_all, ~Bin)

#Estimate the variance between samples
diagdds <- estimateSizeFactors(diagdds,type="poscounts")
diagdds <- estimateDispersions(diagdds) 

# Get variance stabilized data. This reduces "noise" from variation between samples that may be due to differences in amplification or sequencing depth. #
diagvst <- getVarianceStabilizedData(diagdds) 
...
# subset taxa to only parcubacteria # 
phyobj_parcu = subset_taxa(phyobj_all, Class == "Parcubacteria")

p = plot_bar(phyobj_parcu, "Order", fill="Order", facet_grid=~Bin) + geom_bar(aes(color=Order, fill=Order), stat="identity", position="stack") + ggtitle("Parcubacteria Abundances") + ylab ("Abundance (arbitraty units)")
ggsave("parcubacteria_abundance_bin.png", plot = p, path= "./files/images/parcubacteria", height = 3.5, width = 6, units = "in")

p = plot_bar(phyobj_parcu, "Order", fill="Order", facet_grid=~Media_Type_1) + geom_bar(aes(color=Order, fill=Order), stat="identity", position="stack") + ggtitle("Parcubacteria Abundances") + ylab ("Abundance (arbitraty units)")
ggsave("parcubacteria_abundance_mediatype.png", plot = p, path= "./files/images/parcubacteria", height = 3.5, width = 6, units = "in")
