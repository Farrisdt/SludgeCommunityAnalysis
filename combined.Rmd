...
# grab taxons with structural zeroes: this is presence/absence. 0 (struc zero false) is presence, 1 (struc zero true) is absence # 
tab_zero = ancombc_out$zero_ind %>% column_to_rownames(var = "taxon") %>% as.data.frame()
# tab_zero$`structural_zero (Bin = PHA bioplastics reactor)` = tab_zero$`structural_zero (Bin = PHA bioplastics reactor)` %>% as.numeric()
# tab_zero$`structural_zero (Bin = N and P removal)` = tab_zero$`structural_zero (Bin = N and P removal)` %>% as.numeric()
# tab_zero$`structural_zero (Bin = P removal)` = tab_zero$`structural_zero (Bin = P removal)` %>% as.numeric()
