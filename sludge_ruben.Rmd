...
| tidyverse | general data processing | @tidyverse |
ANCOMBC
vegan
DT
phyloseq
...
euler3 = ps_euler(phyobj_all, "Operation", quantities = list(type=c("percent")), fraction = 0, shape = "ellipse", weight = TRUE, relative = TRUE, fill = c("#648FFF", "#785EF0","#DC267F" ))
p3 = grid.arrange(grobs = list(euler3), top = "Operation, Full Sample")
...
ggsave(filename = "euler_operation.png", plot = p3, path = "C:/Users/rlanc/OneDrive/Documents/Bioinformatics/Parcubacteria_project/images")
...
p = subset_samples(ps, Bin == "P removal")
p10 = names(sort(taxa_sums(p), TRUE)[1:10])
pn = subset_samples(ps, Bin == "N and P removal")
pn10 = names(sort(taxa_sums(pn), TRUE)[1:10])
pha = subset_samples(ps, Bin == "PHA bioplastics reactor")
...
```{r ANCOMBC analysis}
# Code courtesy of Dr. Max Wren, modified for this data

## ANalysis of Compositions of Microbiomes with Bias Correction (ANCOM-BC) is an extremely useful form of differential abundance analysis created specifically for short read sequencing. It accounts for sampling bias by estimating the "sampling fraction" which is the ratio of true abundance versus sample abundance based on a linear regression of the observed counts from all samples. 

# working with a non-normalized phyloseq: this one has not been through variance stablization with DESeq2 since error correction is built into ANCOM-BC?
...
# Categorical covariates: “System.Name, System.Type, Media.Type.1, Bin”
